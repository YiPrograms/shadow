...
static void nss_exit(void) {
	if (nss_is_initialized() && subid_nss_db_head) {
		struct subid_nss_db* current = subid_nss_db_head;
		while (current) {
			struct subid_nss_db* next = current->next;
			if (current->ops) {
...
static struct subid_nss_ops *open_and_check_nss_module(const char* libname) {
	void* h;
...
nss_init(const char *nsswitch_path) {
	...
	token = strtok_r(p, delimiters, &saveptr);
	while (token != NULL) {
		if (streq(token, "files")) {
			...
			snprintf(libname, sizeof(libname), "libsubid_%s.so", token);
...
struct subid_nss_db *get_subid_nss_db() {
	nss_init(NULL);
